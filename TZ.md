...
| GET    | `/accounts/{accountId}/users` | Список пользователей аккаунта                                        | `ManageUsers` | 400, 401, 403, 500       |
| PUT    | `/accounts/{accountId}/users/{userId}` | Изменение роли пользователя в аккаунте                      | `ManageUsers` | 400, 401, 403, 404, 500  |
| DELETE | `/accounts/{accountId}/users/{userId}` | Удаление пользователя из аккаунта. Владельца аккаунта удалить нельзя | `ManageUsers` | 400, 401, 403, 404, 409, 500 |
...
| GET    | `/accounts/{accountId}/roles`          | Список ролей аккаунта                              | `ManageAccountRoles` | 400, 401, 403, 500      |
| DELETE | `/accounts/{accountId}/roles/{roleId}` | Удаление роли аккаунта. Нельзя удалить системную роль и роль с назначенными пользователями | `ManageAccountRoles` | 400, 401, 403, 404, 409, 500 |
...
| GET    | `/accounts/{accountId}/user-groups`                            | Список групп аккаунта             | Аутентификация           | 400, 401, 403, 500      |
| DELETE | `/accounts/{accountId}/user-groups/{groupId}`                  | Удаление группы со всем содержимым: участниками, видео и файлами в S3 | `ManageGroups` | 400, 401, 403, 404, 500 |
...
| DELETE | `/accounts/{accountId}/user-groups/{groupId}/members/{userId}` | Удаление пользователя из группы   | `ManageMembers` (группа) | 400, 401, 403, 404, 500 |
...
| GET    | `/accounts/{accountId}/user-groups/roles`       | Список ролей групп аккаунта                | `ManageGroups` | 400, 401, 403, 500      |
| DELETE | `/accounts/{accountId}/user-groups/roles/{roleId}` | Удаление роли группы. Нельзя удалить роль с назначенными участниками | `ManageGroups` | 400, 401, 403, 404, 409, 500 |
...
| GET    | `/accounts/{accountId}/user-groups/{groupId}/videos`       | Список видео группы с метаданными                                        | `VideoWatch` + членство     | 400, 401, 403, 500  |
| PUT    | `/accounts/{accountId}/user-groups/{groupId}/video/{videoId}` | Переименование видео                                                  | `ManageVideo` (группа)      | 400, 401, 403, 404, 409, 500 |
| DELETE | `/accounts/{accountId}/user-groups/{groupId}/video/{videoId}` | Удаление видео, ассетов и файлов из S3                                | `ManageVideo` (группа)      | 400, 401, 403, 404, 500 |

---

### 6.7 Каскадные действия при удалении

#### Удаление пользователя из аккаунта

1. Пользователь удаляется из всех групп аккаунта (`group_members`).
2. Видео, загруженные пользователем, остаются — они принадлежат группе, а не пользователю.
3. Нельзя удалить владельца аккаунта (`is_system = true`).

#### Удаление роли аккаунта

1. Операция блокируется (409), если хотя бы один пользователь аккаунта имеет эту роль.
2. Системную роль (`is_system = true`) удалить нельзя.

#### Удаление группы пользователей

1. Удаляются все записи участников группы (`group_members`).
2. Для каждого видео группы (`user_group_videos`):
   - Удаляются все ассеты видео (`video_assets`).
   - Удаляются соответствующие записи файлов (`files`).
   - Оригинальный и сжатый файлы удаляются из S3.
3. Удаляются все записи видео группы (`user_group_videos`).

#### Удаление роли группы

1. Операция блокируется (409), если хотя бы один участник группы имеет эту роль.

#### Удаление видео

1. Удаляются все ассеты видео (`video_assets`).
2. Удаляются соответствующие записи файлов (`files`).
3. Оригинальный и сжатый файлы (если есть) удаляются из S3.
