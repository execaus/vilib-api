...
func TestHandler_DeleteChapter(t *testing.T) {
	var (
	...
	t.Run("success", func(t *testing.T) {
		...
		svcMock.Chapter.DeleteMock.
			When(minimock.AnyContext, testAccountID, testGroupID, testInitiatorID, testVideoID, testChapterID).
			Then(nil)
	...
	t.Run("invalid video id", func(t *testing.T) {
		...
		badURL := "/api/v1/accounts/" + testAccountID.String() + "/user-groups/" + testGroupID.String() +
			"/video/invalid-uuid/chapters/" + testChapterID.String()
