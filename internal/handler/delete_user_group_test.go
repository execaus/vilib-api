...
func TestHandler_DeleteUserGroup(t *testing.T) {
	var (
	...
	t.Run("success", func(t *testing.T) {
		...
		svcMock.UserGroup.DeleteMock.When(minimock.AnyContext, testInitiatorID, testAccountID, testGroupID).
			Then(nil)
	...
	t.Run("invalid group id", func(t *testing.T) {
		...
		url := "/api/v1/accounts/" + testAccountID.String() + "/user-groups/invalid-uuid"
		req := httptest.NewRequest(http.MethodDelete, url, nil)
		req.Header.Set("Authorization", "Bearer "+testToken)
