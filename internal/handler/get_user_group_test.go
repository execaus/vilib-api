...
func TestHandler_GetUserGroup(t *testing.T) {
	var (
		testAccountID   = uuid.New()
		testGroupID     = uuid.New()
		testInitiatorID = uuid.New()
		testToken       = "valid-token"
		testGroup       = domain.UserGroup{ID: testGroupID, Name: "group1", AccountID: testAccountID}
	)
	...
	t.Run("success", func(t *testing.T) {
		...
		svcMock.UserGroup.GetMock.When(minimock.AnyContext, testInitiatorID, testAccountID, testGroupID).
			Then(testGroup, nil)
		...
		var resp dto.GetUserGroupResponse
		require.NoError(t, json.Unmarshal(w.Body.Bytes(), &resp))
		require.Equal(t, testGroupID, resp.Group.ID)
		require.Equal(t, "group1", resp.Group.Name)
		require.Equal(t, testAccountID, resp.Group.AccountID)
	})

	t.Run("group not in account is not found", func(t *testing.T) {
		mc := minimock.NewController(t)
	...
	t.Run("invalid account id", func(t *testing.T) {
		...
		url := "/api/v1/accounts/invalid-uuid/user-groups/" + testGroupID.String()
		req := httptest.NewRequest(http.MethodGet, url, nil)
		req.Header.Set("Authorization", "Bearer "+testToken)
