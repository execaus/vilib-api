...
func TestHandler_ReactivateUser(t *testing.T) {
	var (
		...
		testUser        = domain.User{ID: testUserID, Email: "user@example.com"}
	)

	setupSuccessTx := func(mc *minimock.Controller) *saga_mocks.TransactableMock {
	...
	t.Run("conflict - already active", func(t *testing.T) {
		mc := minimock.NewController(t)
		defer mc.Finish()
