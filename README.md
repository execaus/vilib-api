...
Переменные окружения ниже добавлены сменой и сбросом пароля (§6 дизайна эпика Э2, поправка
О-1) поверх переменных, описанных в `config/config.go` / `config/defaults.go`:

| Переменная                 | Секция конфига            | Обязательна | По умолчанию | Описание |
|-----------------------------|----------------------------|-------------|--------------|----------|
| `AUTH_PASSWORD_RESET_TTL`   | `auth.password_reset_ttl`  | нет         | `1h`         | Время жизни токена сброса пароля (`POST auth/password/forgot` → `POST auth/password/reset`). |
| `FRONTEND_ORIGIN`           | `frontend.origin`          | **да**      | —            | Базовый адрес веб-интерфейса (без завершающего слеша), используется для ссылки в письме сброса пароля: `${FRONTEND_ORIGIN}/reset-password?token=<token>`. Для локальной разработки — адрес dev-сервера `vilib-fe` (например, `http://localhost:5173`). Отсутствие переменной — ошибка `Config.Validate()`, приложение не стартует. |

Переменные ниже добавлены обязательным обучением с подтверждением просмотра (§3 дизайна
эпика Э3, В-1):

| Переменная                          | Секция конфига                     | Обязательна | По умолчанию | Описание |
|--------------------------------------|-------------------------------------|-------------|--------------|----------|
| `VIDEO_WATCH_COMPLETION_THRESHOLD`   | `video.watch_completion_threshold`  | нет         | `0.95`       | Доля покрытия видео (0;1], после которой просмотр считается подтверждённым. Значение вне `(0;1]` — ошибка `Config.Validate()`. |
| `VIDEO_WATCH_HEARTBEAT_INTERVAL`     | `video.watch_heartbeat_interval`    | нет         | `10s`        | Ожидаемый период отправки heartbeat'ов плеером — базовый интервал для расчёта допустимой длины первого принятого интервала сессии просмотра. |
...
- **финальный** (без `target`) — сам API: `golang:1.25-alpine` → `alpine`, непривилегированный
  ...
  окружения (`config.LoadConfig` умеет работать без файла). Пустой `EMAIL_USERNAME` означает
  отправку почты без SMTP-аутентификации — режим для локального перехватчика писем (например,
  Mailpit), который не поддерживает SMTP AUTH без TLS.
